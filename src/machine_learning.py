...
def get_from_cache(key, calc_func, redis_connection):
    ...
    if redis_connection != None:
        redis_connection.set(key, compress_string(
            json.dumps(result)), ex=7 * 24 * 60 * 60)
...
def string_to_bytes(complete_track, key, other_key, n, redis_connection=None):
    ...
    feature_key = get_feature_key(
        string_to_bytes.__name__, key, other_key, n, complete_track["id"])
...
def get_track_data():
    ...
    n_beats = int(sum(len(i["analysis"]["beats"])
                      for i in tracks_complete) / len(tracks_complete))
    n_bars = int(sum(len(i["analysis"]["bars"])
                     for i in tracks_complete) / len(tracks_complete))
    n_sections = int(sum(len(i["analysis"]["sections"])
                         for i in tracks_complete) / len(tracks_complete))
    n_segments = int(sum(len(i["analysis"]["segments"])
                         for i in tracks_complete) / len(tracks_complete))
    n_tatums = int(sum(len(i["analysis"]["tatums"])
                       for i in tracks_complete) / len(tracks_complete))
    n_synchstring = int(sum(len(i["analysis"]["track"]["synchstring"])
                            for i in tracks_complete) / len(tracks_complete))
    n_codestring = int(sum(len(i["analysis"]["track"]["codestring"])
                           for i in tracks_complete) / len(tracks_complete))
    n_echoprintstring = int(sum(len(i["analysis"]["track"]["echoprintstring"])
                                for i in tracks_complete) / len(tracks_complete))
    n_rhythmstring = int(sum(len(i["analysis"]["track"]["rhythmstring"])
                             for i in tracks_complete) / len(tracks_complete))
...
def create_clf(
    ...
    name = 'clf/{}_clf_{}.bin'.format(int(acc * 1000),
...
def get_feature_set_hash(feature_set):
    ...
    for feature in feature_set:
        parts.append(
            "{}_{}_{}".format(feature["fun"].__name__,
                              feature["key"], feature["sec"])
        )
...
def recreate_training_playlist():
    new_training_set_id, new_training_set_n = get_playlist_by_name(
        "new training set")
    ...
    not_ska_playlist_id, not_ska_playlist_n = get_playlist_by_name(
        NOT_SKA_PLAYLIST)
...
def progress_bar_ml_create(progress, n, acc, delta, delta_avg):
    print_progress_bar(
        ...
        suffix="acc {:.2f} took {:.2f} secs avg {:.2f}".format(
            acc, delta, delta_avg)
    )
...
def create_ml_complete_set(
    ...
    for complete_track in complete_tracks:
        ...
        for feature in complete_feature_set:
            ...
            track_feature[feature_key] = feature["fun"](
                complete_track, key, sec, n, redis_connection)
...
def create_all_clf():
    ...
    num_cores = int(multiprocessing.cpu_count())
    to_remove = Parallel(n_jobs=num_cores)(
        delayed(process)(i, files) for i in feature_sets)
    ...
    print_progress_bar(len(feature_sets), len(
        feature_sets) + 1, prefix="Creating ml sets")
...
def create_fresh_clf():
    ...
    for i in clf_to_test:
        ...
        print("prob of {} being ska is {:.2f}%".format(
            track_info["name"], ska_prob[1] * 100))
